...
pko <- pko %>%
  ...
  dplyr::mutate(start_year = as.numeric(stringr::str_sub(start_date, start = 1, end = 4)),
...
ongoing_pko <- c("UNAMID","MINUSMA","MINURSO","MONUSCO","UNISFA","UNMISS","MINUSCA","UNMOGIP",
                 "UNFICYP","UNMIK","UNTSO","UNDOF","UNIFIL")
...
pko <- pko %>%
  ...
  # drop pkos without an iso3c code associated; this is mostly due to extra coding of missions
  # with multiple parts having an overall mission entry (e.g., UNOSOM I and UNOSOM II also
  # coded as just UNOSOM encompassing both missions)
  tidyr::drop_na(iso3c) %>%
...
pko2$country[pko2$iso3c=="KSV"] <- "KSV"
...
pko2 <- pko2 %>%
  dplyr::full_join(country_years,by=c("iso3c","country","year")) %>%
...
### South Vietnam, the Soviet Union, and Yugoslavia.
# country
### A country's commonly used English-language name.
# year
### The calendar year the specific variable is measured during.
# pko_mission
### A binary variable representing if a United Nations or joint United Nations peacekeeping mission operated within the
