...
aid <- aiddata_full %>%
  ...
  dplyr::filter(
    ...
    recipient %!in% c(
      "Africa, North of Sahara, Regional Programs", "Africa, Regional Programs Multi-Country",
      ...
      "Central Asia, Regional Programs, Regional Programs", "Europe, Regional Programs",
      "European Commission", "Ex-Yugoslavian States, Unspecified",
      "Far East Asia, Regional Programs", "Global",
      "Latin America, Regional Programs, Regional Programs", "MADCT Unspecified",
      "Mediterranean, Regional Programs, Regional Programs", "Middle East, Regional Programs",
      "no value", "North & Central America, Regional Programs",
      "North & Central America, regional, Regional Programs", "Oceania, Regional Programs",
      "South & Central Asia, Regional Programs", "South America, Regional Programs",
  ...
  dplyr::mutate(
    ...
    iso3c = dplyr::case_when(
      recipient == "Kosovo" ~ "KSV",
      recipient == "Serbia and Montenegro" ~ "SRB",
      recipient == "Yugoslavia" ~ "YUG",
      # also recode non-sovereign entitites into the sovereign country
      recipient %in% c("Anguilla", "Bermuda", "Cayman Islands", "Falkland Islands", "Gibraltar",
                       "Monserrat", "St. Helena", "Turks and Caicos Islands",
                       "Virgin Islands (UK)") ~ "GBR",
      recipient %in% c("Aruba", "Netherlands Antilles") ~ "NLD",
      recipient %in% c("Cook Islands", "Niue", "Tokelau") ~ "NZL",
      recipient %in% c("French Guinea", "French Polynesia", "Guadaloupe", "Martinique", "Mayotte",
                       "New Caledonia", "Reunion", "St. Pierre & Miquelon",
                       "Wallis & Futuna") ~ "FRA",
      recipient %in% c("Hong Kong, China", "Macao") ~ "CHN",
      recipient == "Northern Marianas" ~ "USA",
      .default = countrycode::countrycode(recipient,"country.name","iso3c"))
    )

# collapse by country-year
aid <- aid %>%
  dplyr::group_by(iso3c, year) %>%
  dplyr::summarise(commitment_amount_usd_current = sum(commitment_amount_usd_current,
                                                       na.rm = TRUE)) %>%
  dplyr::ungroup()
...
aid <- aid %>%
  ...
  dplyr::mutate(aid_value = ifelse(is.na(aid_value), 0, aid_value))
...
for(y in 1946:1970){
  ...
  aid$aid_value[aid$iso3c == "PAK"] <- sum(aid$aid_value[aid$iso3c %in% c("PAK", "BGD")])
...
aid2 <- aid %>%
  ...
  # dplyr::filter(cn == 1) %>%
  dplyr::mutate(
    aid.perc.gdp.pwt = 100 * aid_value / gdp.pwt.est,
    aid.perc.gdp.gl = 100 * aid_value / gdp.gl.est,
    aid.per.capita.un = aid_value / un.pop,
    aid.per.capita.cow = aid_value / cow.pop
    ) %>%
  dplyr::select(iso3c, year, aid_value, aid.perc.gdp.pwt, aid.perc.gdp.gl, aid.per.capita.un,
                aid.per.capita.cow)

aid2$aid_value[is.na(aid2$aid_value)] <- 0
aid2$aid.perc.gdp.pwt[is.na(aid2$aid.perc.gdp.pwt)] <- 0
aid2$aid.perc.gdp.gl[is.na(aid2$aid.perc.gdp.gl)] <- 0
aid2$aid.per.capita.un[is.na(aid2$aid.per.capita.un)] <- 0
aid2$aid.per.capita.cow[is.na(aid2$aid.per.capita.cow)] <- 0
