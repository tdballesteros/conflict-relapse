...
### format data ----------------------------------------------------------------------
ucdp <- ucdp_original %>%
  dplyr::select(conflict_id,side_a,year,intensity_level,type_of_conflict) %>%
  dplyr::rename(confid = conflict_id,
                country = side_a,
  ...
  dplyr::mutate(country = gsub(pattern = "Government of ",
                               replacement = "",
                               country),
...
# add iso3c codes for missing values
ucdp$iso3c[ucdp$country=="Hyderabad"] <- "IND"
ucdp$iso3c[ucdp$country=="South Vietnam"] <- "RVN"
ucdp$iso3c[ucdp$country=="South Yemen"] <- "YPR"
ucdp$iso3c[ucdp$country=="Yemen (North Yemen)"&ucdp$year<1991] <- "YAR"
ucdp$iso3c[ucdp$country=="Yemen (North Yemen)"&ucdp$year>=1991] <- "YEM"
...
# add country names for missing values
ucdp$country[ucdp$iso3c=="RVN"] <- "South Vietnam"
ucdp$country[ucdp$iso3c=="YAR"] <- "North Yemen"
ucdp$country[ucdp$iso3c=="YPR"] <- "South Yemen"
...
ucdp <- ucdp %>%
  ...
  dplyr::mutate(conflict = ifelse(intensity %in% c(1:2),1,0),
...
# add iso3c and country names to newly expanded dataset
ucdp <- ucdp %>%
  dplyr::full_join(list_of_conflicts_and_countries,by="confid") %>%
  dplyr::mutate(iso3c = dplyr::coalesce(iso3c.x,iso3c.y),
                country = dplyr::coalesce(country.x,country.y)) %>%
  dplyr::select(-c(iso3c.x,iso3c.y,country.x,country.y))
...
ucdp5 <- ucdp4 %>%
  ...
  dplyr::mutate(years = paste(min(year),"-",max(year)),
  ...
  dplyr::full_join(list_of_conflicts_and_countries,by="confid") %>%
  dplyr::mutate(yrstart = as.numeric(stringr::str_sub(years, start = 1, end = 4)),
