...
fiscap <- dplyr::full_join(tax_revenue, relative_capacity, by = c("iso3c", "year")) %>%
  ...
  dplyr::mutate(gdppc.pwt.un = gdp.pwt.est / un.pop, # gdp per capita
                lngdppc = log(gdppc.pwt.un), # natural log of gdp per capita
                aidpc = aid_value / un.pop, # aid per capita
                lnaidpc = log(aidpc), # natural log of aid per capita
                # lnaidgdp = log(aid.perc.gdp.pwt), # natural log of aid as a proportion of gdp
                lntaxgdp = log(taxgdp)
                ) %>% # natural log of tax as a proportion of gdp
...
fiscap_pca_variables <- c("taxgdp", "rpe_gdp", "aid.perc.gdp.pwt")
...
fiscap_pca_df <- fiscap %>%
  ...
  dplyr::select(iso3c, year, all_of(fiscap_pca_variables)) %>%
  dplyr::filter(
    taxgdp %!in% c(-Inf, Inf, NA, NaN),
    rpe_gdp %!in% c(-Inf, Inf, NA, NaN),
    aid.perc.gdp.pwt %!in% c(-Inf, Inf, NA, NaN)
  )

fiscap_cor <- cor(fiscap_pca_df %>%
                    dplyr::select(-c(iso3c, year)))

fiscap_pca <- stats::prcomp(
  ~ taxgdp + rpe_gdp + aid.perc.gdp.pwt,
  data = fiscap_pca_df, retx = T, center = T, scale. = T)
summary(fiscap_pca)
...
fiscap_pca_df <- fiscap_pca_df %>%
  ...
  dplyr::select(-c(PC2,PC3)) %>%
  # renames first principal component as mil.cap
  dplyr::rename(fis.cap = PC1) %>%
  ...
  dplyr::mutate(fis.cap.sq = fis.cap^2)

# test directionality of metric
# invert direction if OMN 1975 is negative - higher value is higher capacity.
if(fiscap_pca_df$fis.cap[fiscap_pca_df$iso3c=="OMN"&fiscap_pca_df$year==1975]<1){
  fiscap_pca_df <- fiscap_pca_df %>%
    dplyr::mutate(fis.cap = -1 * fis.cap)
}

#### write data ----------------------------------------------------------------------
# writes formatted dataframe as csv files
write.csv(fiscap_pca_df,"Data files/Formatted data files/fiscal_capacity_index.csv",row.names = FALSE)
